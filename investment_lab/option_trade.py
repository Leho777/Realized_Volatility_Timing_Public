...
class OptionTradeABC(ABC):
    ...
    def _generate_trades(
        ...
        if "risk_free_rate" not in df_trades_daily.columns:
            ...
            try:
                ...
                logging.warning(
                    "Skipping rate/forward enrichment for %s to %s: %s",
                    start,
                    end,
                    exc,
                )
    ...
    def _select_options(
        ...
        for leg in deepcopy(legs):
            ...
            df_list.append(
                selected_option_df.rename(columns={"date": "entry_date"})
            )
    ...
    def _neutralize_cost(cls, df_trades: pd.DataFrame) -> pd.DataFrame:
        ...
        df_trades_cp["weight"] = np.where(
            (
                (df_trades_cp["missing_premium"] < 0)
                & (df_trades_cp["weight"] < 0)
            )
            | (
                (df_trades_cp["missing_premium"] > 0)
                & (df_trades_cp["weight"] > 0)
    ...
    def _convert_trades_to_timeseries(cls, df_trades: pd.DataFrame) -> pd.DataFrame:
        logging.info(
            "Converting %s df_trades to daily time series", len(df_trades)
        )
        df_trades_cp = df_trades.copy()
...
class VarianceSwap(DeltaHedgedOptionTrade):
    ...
    def _select_options(
        ...
        **kwargs,
        ...
        df_list = []
        check_is_true(
            len(legs) == 1, "Error a VarianceSwap class can only handle 1 leg."
        )
        for leg in deepcopy(legs):
            ...
            selected_option_df["leg_name"] = leg_name
            selected_option_df["strike_weight"] = strike_spacing / (
                selected_option_df["strike"] ** 2
            )
            selected_option_df = selected_option_df.groupby(
                ["date", "ticker"], group_keys=False
            ).apply(
    ...
    def _normalize_strike_weights(
        ...
        if strike_weight_sum > 0:
            ...
            df_group["weight"] = (
                df_group["strike_weight"] * normalization_factor
            )
        else:
            df_group["weight"] = 0
        return df_group
