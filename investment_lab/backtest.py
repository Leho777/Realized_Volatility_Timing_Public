...
class StrategyBacktester:
    ...
    def compute_backtest(self, tcost_args: Optional[dict[str, Any]] = None) -> Self:
        ...
        df_positions["prev_theta"] = (
            df_positions.groupby("option_id")["theta"].shift(1).bfill()
        )
        df_positions["prev_gamma"] = (
            df_positions.groupby("option_id")["gamma"].shift(1).bfill()
        )
        df_positions["prev_delta"] = (
            df_positions.groupby("option_id")["delta"].shift(1).bfill()
        )
        df_positions["prev_vega"] = (
            df_positions.groupby("option_id")["vega"].shift(1).bfill()
        )
        df_positions["prev_rho"] = (
            df_positions.groupby("option_id")["rho"].shift(1).bfill()
        )
    ...
    def __del__(self):
        try:
            ...
            # Interpreter shutdown can leave pandas internals partially unloaded.
            pass
