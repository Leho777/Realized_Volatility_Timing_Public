...
class OptionLoader(DataLoader):
    @classmethod
    def _default_ticker(cls) -> str | None:
        return None

    @classmethod
    def _get_path(cls) -> str:
        return _resolve_data_path("optiondb_2016_2023.parquet")
    ...
    def _process_loaded_data(
        ...
        if "ticker" not in df.columns:
            default_ticker = cls._default_ticker()
            if default_ticker is None:
                raise ValueError(
                    "Option data is missing the 'ticker' column and no default ticker "
                    "is defined for this loader."
                )
            df = df.copy()
        ...
        df = df[df["ticker"].isin(ticker)].copy()
...
class SPYOptionLoader(OptionLoader):
    @classmethod
    def _default_ticker(cls) -> str:
        return "SPY"

    @classmethod
    def _get_path(cls) -> str:
        return _resolve_data_path("spy_2020_2022.parquet")
...
class AAPLOptionLoader(OptionLoader):
    ...
    @classmethod
    def _get_path(cls) -> str:
        return _resolve_data_path("aapl_2016_2023.parquet")
